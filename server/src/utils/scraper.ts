...
export async function runGoogleScraper(buissnessName: string): Promise<returnDataType[]> {
    ...
    try {
        ...
        const firstHeading = await driver.wait(
            until.elementLocated(By.css('html > body > div:nth-of-type(3) > div > div:nth-of-type(13) > div > div:nth-of-type(2) > div:nth-of-type(2) > div > div > div:first-of-type > div > div:nth-of-type(4)')),
            15000
        );
        ...
        for (const div of elementsLocated) {
            ...
            if (anchors.length > 0) {
                ...
                if (text !== "" && !href?.includes("instagram.com") && !href?.includes("facebook.com")) {
                    ...
                    try {
                        const phone = await driver.wait(
                            until.elementLocated(By.css("a[data-phone-number]")),
                            2000
                        );
                        ...
                        try {
                            ...
                            const item: returnDataType = { name }
                            if (phoneNumber) {
    ...
    const nameMap = new Map<string, returnDataType>();
    const phoneMap = new Map<string, returnDataType>();
    ...
    const finalResults: returnDataType[] = [];
